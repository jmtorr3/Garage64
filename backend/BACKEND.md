...
```
TODO:
- [] seems to have saved the entire jem to the backend here, may make it into a template, but there's a bug that only updates the database, but not the actual .jem saved in the folder
